...
// Mulai session untuk mendapatkan data user yang login
if (session_status() == PHP_SESSION_NONE) {
    session_start();
}

// Konsistensi pengambilan user ID
$user_id = null;
if (isset($_SESSION['id_user'])) {
    $user_id = $_SESSION['id_user'];
} else {
    // Untuk testing, gunakan user ID default yang sama dengan baca.php
    $user_id = 12311;
}
...
$totalBooksQuery = $conn->query("SELECT COUNT(*) as total FROM history WHERE id_user = $user_id");
$totalBooks = $totalBooksQuery ? $totalBooksQuery->fetch_assoc()['total'] : 0;
...
");
$monthlyCount = $monthlyQuery ? $monthlyQuery->fetch_assoc()['monthly_count'] : 0;

$favCategoryQuery = $conn->query("
...
");
$favCategory = "N/A";
if ($favCategoryQuery && $favCategoryQuery->num_rows > 0) {
    $favData = $favCategoryQuery->fetch_assoc();
    $favCategory = $favData['nama_kategori'] ? ucfirst($favData['nama_kategori']) : "N/A";
}
?>
